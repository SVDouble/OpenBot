...
from sismic.interpreter.default import _KeyifyList
from sismic.interpreter.listener import (
...
class AsyncInterpreter:
    ...
    def __init__(
        ...
        *,
        evaluator_klass: Callable[..., AsyncEvaluator] = AsyncPythonEvaluator,
        ...
        asyncio.create_task(self._evaluator.execute_statechart(statechart))
    ...
    def configuration(self) -> List[str]:
        ...
        """
        return sorted(
            self._configuration, key=lambda s: (self._statechart.depth_for(s), s)
        )
    ...
    def bind_property_statechart(
        self, statechart: Statechart, *, interpreter_klass: Callable = None
    ) -> Callable[[MetaEvent], Any]:
        """
    ...
    async def execute(self, max_steps: int = -1) -> List[MacroStep]:
        """
        ...
        returned_steps = []
        i = 0
        macro_step = await self.execute_once()
        while macro_step:
    ...
    async def execute_once(self) -> Optional[MacroStep]:
        """
        ...
        if len(computed_steps) > 0:
            ...
            for step in computed_steps:
                executed_steps.append(await self._apply_step(step))
                executed_steps.extend(await self._stabilize())
        ...
        configuration = (
            self.configuration
        )  # Use self.configuration to benefit from the sorting
        ...
        *,
        ...
        for transition in self._statechart.transitions:
            if transition.source in states:
                if transition.event is None or transition.event == getattr(
                    event, "name", None
                ):
                    ...
                    if transition.source not in _state_depth_cache:
                        _state_depth_cache[
                            transition.source
                        ] = self._statechart.depth_for(transition.source)
                    ...
                    for _, transitions in sorted_groupby(
                        ...
                        for transition in transitions:
                            if (
                                ...
                                or await self._evaluator.evaluate_guard(
    ...
    def _sort_transitions(self, transitions: List[Transition]) -> List[Transition]:
        ...
        if len(transitions) > 1:
            ...
            for t1, t2 in combinations(transitions, 2):
                # Check (1)
                lca = cast(
                    str, self._statechart.least_common_ancestor(t1.source, t2.source)
                )
                ...
                for transition in [t1, t2]:
                    ...
                    if transition.target and (
                        transition.target
                        not in [last_before_lca]
                        + self._statechart.descendants_for(last_before_lca)
                    ):
    ...
    async def _compute_steps(self) -> List[MicroStep]:
        """
        ...
        transitions = await self._select_transitions(event, states=self._configuration)
    ...
    def _create_steps(
        self, event: Optional[Event], transitions: Iterable[Transition]
    ) -> List[MicroStep]:
        """
        ...
        for transition in transitions:
            ...
            lca = self._statechart.least_common_ancestor(
    ...
    def _create_stabilization_step(self, names: Iterable[str]) -> Optional[MicroStep]:
        ...
        for leaf in leaves:
            if (
                ...
                return MicroStep(
                    exited_states=[leaf.name, cast(str, self._statechart.root)]
                )
            if isinstance(leaf, (ShallowHistoryState, DeepHistoryState)):
                states_to_enter = cast(
                    List[str], self._memory.get(leaf.name, [leaf.memory])
                )
                states_to_enter.sort(key=lambda x: (self._statechart.depth_for(x), x))
                return MicroStep(
                    entered_states=states_to_enter, exited_states=[leaf.name]
                )
            elif isinstance(leaf, OrthogonalState) and self._statechart.children_for(
                leaf.name
            ):
                return MicroStep(
                    entered_states=sorted(self._statechart.children_for(leaf.name))
                )
    ...
    async def _apply_step(self, step: MicroStep) -> MicroStep:
        """
        ...
        for state in exited_states:
            # Execute exit action
            sent_events.extend(await self._evaluator.execute_on_exit(state))
        ...
        if step.transition:
            ...
            await self._evaluate_contract_conditions(
                step.transition, "preconditions", step
            )
            await self._evaluate_contract_conditions(
                step.transition, "invariants", step
            )

            sent_events.extend(
                await self._evaluator.execute_action(step.transition, step.event)
            )

            # Postconditions and invariants
            await self._evaluate_contract_conditions(
                step.transition, "postconditions", step
            )
            await self._evaluate_contract_conditions(
                step.transition, "invariants", step
            )
        ...
        for state in entered_states:
            # Preconditions
            await self._evaluate_contract_conditions(state, "preconditions", step)

            # Execute entry action
            sent_events.extend(await self._evaluator.execute_on_entry(state))
    ...
    async def _stabilize(self) -> List[MicroStep]:
        """
        ...
        while step is not None:
            steps.append(await self._apply_step(step))
            step = self._create_stabilization_step(self._configuration)
        return steps

    async def _evaluate_contract_conditions(
        self,
        obj: Union[Transition, StateMixin],
        cond_type: str,
        step: Optional[Union[MacroStep, MicroStep]] = None,
    ) -> None:
        """
        ...
        unsatisfied_conditions = await getattr(
            self._evaluator, "evaluate_" + cond_type
        )(obj, getattr(step, "event", None))
